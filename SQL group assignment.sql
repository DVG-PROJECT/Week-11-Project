...
SHOW COLUMNS FROM fashion;

-- SALES ANALYSIS --
-- 1. Identifying the top 5 selling products  --
...
select TO_CHAR(order_date,'month') as month, SUM (revenue) as total_revenue -- for extract we can use the extract from 
from fashion
group by TO_CHAR(order_date,'month')
;

-- 3. Analyse sales distribution by day of the week
select TO_CHAR(order_date,'DAY') as month, SUM (revenue) as total_revenue -- for extract we can use the extract from 
from fashion
...
-- Insight: There are no new customers

-- 3. Identify Locations with the most active buyers #if applicable
select shop_outlet, count (*) as active_buyers
from fashion 
group by shop_outlet
order by active_buyers desc;
-- insight: Brightbean has the most active buyers
...
select 
	case 
		when trim( TO_CHAR(order_date,'DAY')) in ('SATURDAY','SUNDAY') then 'weekend' -- trim removes the white spaces 
		else 'weekday'
		end as day_s, sum (revenue) as total_revenue
from fashion 
group by day_s;

-- 2. find peak shopping hours  # if timestamp is available
-- insight: we dont have any column that has time


-- D. Inventory Insights
-- 1. Identify low stock items.
select category as category_distinct, count(*) as total_count
from fashion 
group by category 
order by total_count asc
-- Insight: shirts are the low stock items


-- Find the items that are frequently restocked
select category as category_distinct, count(*) as total_count
from fashion 
group by category 
order by total_count desc
-- Insight: shoes are ordered more hence are restocked more

-- E. Custom Question
-- what is the average revenue for each category
select category,round( AVG (revenue),2) as Average_revenue
from fashion 
group by category 
